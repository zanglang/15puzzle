...
public final class PuzzleState implements State {
	...
	private static Action[] actionSequence = { MOVE_LEFT, MOVE_RIGHT, MOVE_UP,
			MOVE_DOWN };
	...
	public static void performAction(PuzzleState origin, Action action) {
		...
		// Check which action is taken, check if it is "valid", if so carry out
		// the action (change the board)
		if (action == MOVE_UP && rowEmpty != 0) {
			origin.tiles[rowEmpty][colEmpty] = origin.tiles[rowEmpty - 1][colEmpty]; // fill
																						// empty
																						// spot
																						// with
																						// moved
	...
	/**
	 * Check if this state is a goal state: |01 02 03 04| |05 06 07 08| |09 10
	 * 11 12| |13 14 15 00|
	 * 

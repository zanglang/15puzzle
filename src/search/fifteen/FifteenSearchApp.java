...
public class FifteenSearchApp {
	...
	public static void main(String[] args) {
		...
		// empty), and
		// pull out the actions that were used to generate this goal state from
		// the initial state
		/*
		 * Action[] actions1G = solveH1G(new PuzzleState(myState)); Action[]
		 * actions1A = solveH1A(new PuzzleState(myState)); Action[] actions2G =
		 * solveH2G(new PuzzleState(myState));
		 */
		...
		/*
		 * System.out.println("Solution via H1 with Greedy:-------------"); for
		 * (int i=0; i<actions1G.length; i++) { System.out.println((i+1)+":
		 * "+actions1G[actions1G.length-1-i]);
		 * PuzzleState.performAction(myState2,actions1G[actions1G.length-1-i]);
		 * System.out.println(myState2.toString()); }
		 * 
		 * System.out.println("Solution via H1 with A*:-------------"); myState2 =
		 * new PuzzleState(myState); for (int i=0; i<actions1A.length; i++) {
		 ...
		 * myState2 = new PuzzleState(myState); for (int i=0; i<actions2G.length;
		 * i++) { System.out.println((i+1)+":
		 * "+actions2G[actions2G.length-1-i]);
		...
		for (int i = 0; i < actions2A.length; i++) {
			System.out.println((i + 1) + ": " +
					actions2A[actions2A.length - 1 - i]);
			PuzzleState.performAction(myState2,
					actions2A[actions2A.length - 1 - i]);
			System.out.println(myState2.toString());
		}
		...
		for (int i = 0; i < actions3A.length; i++) {
			System.out.println((i + 1) + ": " +
					actions3A[actions3A.length - 1 - i]);
			PuzzleState.performAction(myState2,
					actions3A[actions3A.length - 1 - i]);
			System.out.println(myState2.toString());
		}
	...
	/**
	 * Example solve You must change this function to solve the problem with
	 * your own Greedy implementation with heuristic function 1.
	 * 
	 * @param state
	 *            initial puzzle state
	 */
	...
	/**
	 * Example solve You must change this function to solve the problem with
	 * your own A* implementation with heuristic function 1.
	 * 
	 * @param state
	 *            initial puzzle state
	 */
	...
	/**
	 * Example solve You must change this function to solve the problem with
	 * your own Greedy implementation with heuristic function 2.
	 * 
	 * @param state
	 *            initial puzzle state
	 */
	...
	/**
	 * Example solve You must change this function to solve the problem with
	 * your own A* implementation with heuristic function 2.
	 * 
	 * @param state
	 *            initial puzzle state
	 */
	...
	public static Action[] solveH3A(PuzzleState state) {
		Node goal = MisplacedTilesSearch.traverse(state);
		Action[] actions = goal.getActions();
	 ...
	 * 
	 * @param maxShuffles
	 *            the number of shuffles to be performed
	 */
